...
const { triggerLabel, queueLabel, firstInQueueLabel, fullQueueLabel, coreApprovalLabel } = require('../util/queueLabels');
...
module.exports = async context => {
    ...
    const isQueuedLabelAddedWithoutReadyToMerge = checkIfQueuedLabelIsAddedWithoutReadyForMergeLabel(context, ownerRepoNumberInfo, labelsOnPR);

    if (isQueuedLabelAddedWithoutReadyToMerge) {
...
function checkIfQueuedLabelIsAddedWithoutReadyForMergeLabel (context, ownerRepoNumberInfo, labelsOnPR) {
    const checkIfQueuedLabelIsAdded = context.payload.label.name.includes('QUEUED FOR MERGE');
    const checkIfPRContainsReadyForMergeLabel = labelsOnPR.data.filter(label => label.name === triggerLabel).length > 0;

    return checkIfQueuedLabelIsAdded && !checkIfPRContainsReadyForMergeLabel;
}

async function sendQueueLabelCommentToPR (context) {
    const queueLabelComment = 'The `QUEUED FOR MERGE` label should only be added by the queue-manager bot. ' +
      'Please remove the `QUEUED FOR MERGE` label from this PR. ' +
...
async function sendCoreApprovalCommentNeededToPR (context) {
    const coreApprovalComment = 'Sorry, this PR needs to have the `CORE APPROVED` label first before adding the `READY FOR MERGE` label. ' +
      'Please remove the `READY FOR MERGE` label from the PR and add the `CORE APPROVED` label first. ' +
      'A core approver must approve this PR before adding the `CORE APPROVED` label.';

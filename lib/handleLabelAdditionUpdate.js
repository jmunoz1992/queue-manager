...
module.exports = async context => {
    ...
    if (context.payload.label.name === triggerLabel) {
        ...
        const coreApprovalCheck = await checkCoreApprovedLabelIsOnPR(context, ownerRepoNumberInfo);

        if(!coreApprovalCheck) {
            const comment = 'Sorry, this PR needs to have the `CORE_APPROVED` label first before adding the `READY_FOR_MERGE` label. ' +
              'A core approver must approve this PR and add the `CORE_APPROVED` label.';
            const commentLabel = context.issue({ body: comment });
